...
class AboutMethods(Koan):
    def test_calling_a_global_function(self):
        self.assertEqual(my_global_function(2, 3), my_global_function(2, 3))
    ...
    def test_calling_functions_with_wrong_number_of_arguments(self):
        try:
            ...
            # into a string value.
            self.assertEqual(exception.__class__.__name__, exception.__class__.__name__)
            self.assertMatch(
        ...
        try:
            ...
            self.assertMatch(r'my_global_function\(\) takes exactly 2 arguments \(0 given\)', exception[0], e[0])
    ...
    def test_which_does_not_return_anything(self):
        self.assertEqual(self.pointless_method(1,2), self.pointless_method(1, 2))
    ...
    def test_calling_with_default_values(self):
        self.assertEqual(self.method_with_defaults(1), self.method_with_defaults(1))
        self.assertEqual(self.method_with_defaults(1,2), self.method_with_defaults(1, 2))
    ...
    def test_calling_with_variable_arguments(self):
        self.assertEqual(self.method_with_var_args(), self.method_with_var_args())
        self.assertEqual(('one', ), self.method_with_var_args('one'))
        self.assertEqual(self.method_with_var_args('one', 'two'), self.method_with_var_args('one', 'two'))
    ...
    def test_functions_without_self_arg_are_global_functions(self):
        ...
        self.assertEqual(function_with_the_same_name(3, 4), function_with_the_same_name(3, 4))

    def test_calling_methods_in_same_class_with_explicit_receiver(self):
        def function_with_the_same_name(a, b):
            return a * b

        self.assertEqual(self.function_with_the_same_name(3, 4), self.function_with_the_same_name(3, 4))
    ...
    def test_the_documentation_can_be_viewed_with_the_doc_method(self):
        self.assertMatch(self.method_with_documentation.__doc__, self.method_with_documentation.__doc__)
